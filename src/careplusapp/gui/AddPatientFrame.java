...
public class AddPatientFrame extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        txtPId.setFont(new java.awt.Font("Century Gothic", 0, 18)); // NOI18N
        jPanel1.add(txtPId, new org.netbeans.lib.awtextra.AbsoluteConstraints(320, 110, 250, 50));
    ...
    private void AddBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AddBtnActionPerformed
     ...
     if(isContactNoValid()==false){
        ...
        return;
         ...
         pat.setDoctorId(jcDocId.getSelectedItem().toString().trim().toUpperCase());
         pat.setPatientId(pId);
         pat.setFirstName(fName);
         pat.setLastName(sName);
         pat.setMno(txtMno.getText().toString());
         pat.setAddress(address);
         pat.setCity(city);
         pat.setOpd(opd);
         pat.setAge(age);
         pat.setGender(jcGender.getSelectedItem().toString().trim().toUpperCase());
         pat.setMarriedStatus(jcStatus.getSelectedItem().toString().trim().toUpperCase());
         pat.setAptStatus("REQUEST");
        try {
            pat.setDate(dateConv());
        } catch (ParseException ex) {
            ex.printStackTrace();
        }
        AppointmentPojo app = new AppointmentPojo();
        app.setPatientId(pId);
        app.setPatientName(fName+" "+sName);
        app.setStatus("REQUEST");
        try {
            app.setDoctorName(DoctorsDao.getDoctorNameById(jcDocId.getSelectedItem().toString().trim().toUpperCase()));
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
        app.setOpd(opd);
        app.setMobileNo(txtMno.getText().toString());
        app.setAppointmentDate(getDateTime());
        new VerifyOtpFrame(pat,app).setVisible(true);
        this.dispose();
    ...
    private void loadPatientId() {
        try{
            String pId=PatientDao.getNewPatientId();
            txtPId.setText(pId);
        }
        catch(SQLException ex){
    ...
    private boolean validateInputs() {
        ...
        if(pId.isEmpty() || fName.isEmpty() || sName.isEmpty() || date.isEmpty() || address.isEmpty() || city.isEmpty() || opd.isEmpty()){
            return false;   
        }
        return true;
    }
